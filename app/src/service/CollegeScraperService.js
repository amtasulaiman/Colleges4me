...
const filename = './src/service/colleges.txt';
...
async function scrapeCdCom(collegeInfo, cdURL){
    try {
        ...
        collegeInfo.type =  $('html').find("#profile-overview > div.statbar > div:nth-child(1)").text().trim(); //added by amta
        collegeInfo.avg_gpa = $('html').find("#profile-overview > div:nth-child(4) > div > dl:nth-child(4) > dd:nth-child(2)").text();
        collegeInfo.grad_dept = $('html').find("#profile-money > div:nth-child(3)  > div >dl:nth-child(8) > dd:nth-child(4)").text().trim(); //aded by amta
    } catch (error){
