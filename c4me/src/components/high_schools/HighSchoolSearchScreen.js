...
class HighSchoolSearchScreen extends React.Component {
  ...
  //  console.log(list); 
    ...
    list = list.splice(0,list.length/2);
    let newList = {};
    list.map((data)=>{
      console.log(data);
...
findSimilarity(hsDict){
  ...
  for (var key in hsDict) {
    ...
    if(!(temp.AvgACTComposite == "N/A") && !(this.state.AvgACTComposite == "N/A"))
      similarity += this.compareScore(hsDict[key].AvgACTComposite, this.state.AvgACTComposite, 36.00);
    else
      similarity += 0.70;

    console.log("NOW SIMILARITY = ", similarity);
    if(!(temp.ApOffered == "N/A") && !(this.state.ApOffered == "N/A"))
      similarity += this.compareScore(hsDict[key].ApOffered, this.state.ApOffered, 38.00);
    else
      similarity += 0.70;

      console.log("NOW SIMILARITY = ", similarity);
    if(!(temp.AvgSATEBRW == "N/A") && !(this.state.AvgSATEBRW == "N/A"))
      similarity += this.compareScore(hsDict[key].AvgSATEBRW, this.state.AvgSATEBRW, 800.00);
    else
      similarity += 0.70;

      console.log("NOW SIMILARITY = ", similarity);
    if(!(temp.AvgSATMath) == "N/A" && !(this.state.AvgSATMath == "N/A"))
      similarity += this.compareScore(hsDict[key].AvgSATMath, this.state.AvgSATMath, 800.00);
    else
      similarity += 0.70;

      console.log("NOW SIMILARITY = ", similarity);
    if(!(temp.GradRate == "N/A") && !(this.state.GradRate == "N/A"))
      similarity += this.compareScore(hsDict[key].GradRate, this.state.GradRate, 100.00);
    else
      similarity += 0.70;

      console.log("NOW SIMILARITY = ", similarity);
    if(!(temp.Ranking == "N/A") && !(this.state.Ranking == "N/A"))
              ...
              <table className="striped">
                <thead>
                  <tr>
                      ...
                      <th className="search-header">Graduation Rate</th>
                      <th className="search-header">Avg SAT</th>
                      <th className="search-header">Avg ACT</th>
                      <th className="search-header">AP Classes Offered</th>
                ...
                <tbody>
                  {Object.keys(this.state.highschools).map((highschool) => {
                    ...
                    return  <tr className="search-item">
                                ...
                                {temp["GradRate"] ? (<td className="search-item">{temp["GradRate"]}%</td>) : (<td className="search-item">N/A</td>)}
                                ...
                                {temp["ApOffered"] ? (<td className="search-item">{temp["ApOffered"]}</td>) : (<td className="search-item">N/A</td>)}
